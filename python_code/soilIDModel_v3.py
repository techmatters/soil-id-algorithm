...
def getSoilLocationBasedGlobal(lon, lat, plot_id):
    ...
    wise_data = extract_WISE_data(lon, lat, file_path="%s/wise30sec_poly_simp_soil.shp"%current_app.config['DATA_BACKEND'], layer_name=None, buffer_size=0.5)
    ...
    for (mukey, cokey), group in mucompdata_grouped:
        ...
        loc_scores.append({
            "Cokey": cokey,
            "Mukey": mukey,
            "distance_score": round(loc_score, 3)
        })
    ...
    columns_to_select = ['COMPID', 'TopDep', 'BotDep', 'id', 'Layer', 'SDTO', 'STPC', 
                         'CLPC', 'CFRAG', 'CECS', 'PHAQ', 'ELCO', 'PROP', 'SU_name', 'FAO_SYS']
    new_column_names = ['cokey', 'hzdept_r', 'hzdepb_r', 'chkey', 'hzname', 'sandtotal_r', 'silttotal_r', 
                        'claytotal_r', 'total_frag_volume', 'CEC', 'pH', 'EC', 'comppct_r', 'compname', 'fss']
    ...
    muhorzdata_pd[['hzdept_r', 'hzdepb_r']] = muhorzdata_pd[['hzdept_r', 'hzdepb_r']].fillna(0).astype(int)
    ...
    for name, count in name_counts.items():
        if count > 1:
            for suffix in range(1, count + 1):
                mucompdata_pd.loc[mucompdata_pd["compname"] == name, "compname"] = name + str(suffix)
    
    # Add modified compname to muhorzdata
    muhorzdata_name = muhorzdata_pd[['cokey']].merge(mucompdata_pd[['cokey', 'compname']], on='cokey')
    ...
    for group in muhorzdata_group_cokey:
        profile = group.sort_values(by='hzdept_r').drop_duplicates(keep='first').reset_index(drop=True)
        ...
        combined_data.columns = ["sandpct_intpl", "claypct_intpl", "rfv_intpl", "compname", "cokey", "comppct"]
        ...
        def aggregated_data_layer(data, column_name):
            return agg_data_layer(data=data[column_name], bottom=c_bottom_temp["c_very_bottom"].iloc[0], depth=True)
        ...
        txt_d = [getTexture(row=None, sand=s, silt=(100-(s + c)), clay=c) for s, c in zip(snd_d, cly_d)]
        ...
        for data_list, data in zip([snd_lyrs, cly_lyrs, txt_lyrs, rf_lyrs, cec_lyrs, ph_lyrs, ec_lyrs, hz_lyrs],
    ...
    mucompdata_pd = pd.merge(mucompdata_pd, c_bottom_depths[['compname', 'c_very_bottom']], on='compname', how='left')
    ...
    #------------------------------------------------------------
    # Format output data
    soilIDRank_output = [group[['compname', 'sandpct_intpl', 'claypct_intpl', 'rfv_intpl']] for group in getProfile_cokey]
    soilIDRank_output_pd = pd.concat(soilIDRank_output, axis=0).reset_index(drop=True)
    
    mucompdata_cond_prob = mucompdata_pd.sort_values('distance_score_norm', ascending=False).reset_index(drop=True)
    ...
    WRB_Comp_Desc = getWRB_descriptions(mucompdata_cond_prob["compname_grp"].drop_duplicates().tolist())
    mucompdata_cond_prob = pd.merge(mucompdata_cond_prob, WRB_Comp_Desc, left_on='compname_grp', right_on='WRB_tax', how='left')
    
    mucomp_index = mucompdata_cond_prob.sort_values(['soilID_rank', 'distance_score_norm'], ascending=[False, False]).index
    ...
    Site = [{
        ...
        "siteDescription": {
            key: row[key] for key in ["Description_en", "Management_en", "Description_es", "Management_es", "Description_ks", "Management_ks", "Description_fr", "Management_fr"]
        }
    ...
    output_SoilList = [
        dict(zip(
            ["id", "site", "bottom_depth", "sand", "clay", "texture", "rock_fragments", "cec", "ph", "ec"],
            item
        ))
        for item in zip(ID, Site, hz_lyrs, snd_lyrs, cly_lyrs, txt_lyrs, rf_lyrs, cec_lyrs, ph_lyrs, ec_lyrs)
    ]
    
    # Save data
    if plot_id is None:
        soilIDRank_output_pd.to_csv("%s/soilIDRank_ofile1.csv" % current_app.config['DATA_BACKEND'], index=None, header=True)
        mucompdata_cond_prob.to_csv("%s/soilIDRank_ofile2.csv" % current_app.config['DATA_BACKEND'], index=None, header=True)
    else:
        saveModelOutput(
            plot_id, model_version,
            json.dumps({
                "metadata": {
                    "location": "global", "model": "v2",
                    "unit_measure": {
                        "distance": "m", "depth": "cm", "cec": "cmol(c)/kg",
                        "clay": "%", "rock_fragments": "cm3/100cm3", "sand": "%",
                        "ec": "ds/m"
                    }
                },
...
#####################################################################################################
def rankPredictionGlobal(lon, lat, soilHorizon, horizonDepth,  rfvDepth, lab_Color, bedrock, cracks, plot_id=None):
    ...
    if soil_df_slice is not None:
        ...
        if bedrock and soil_df_slice['bottom'].iloc[-1] > bedrock:
            ...
            for j in range(len(soil_df) - 1):
                if soil_df['top'].iloc[j + 1] > soil_df['bottom'].iloc[j]:
                    ...
                    soil_df = pd.concat([soil_df, layer_add], axis=0).sort_values('top').reset_index(drop=True)
        ...
        horizonDepthB, horizonDepthT = list(map(int, soil_df['bottom'])), list(map(int, soil_df['top']))
        ...
        p_sandpct_intpl = [spt[i] for i in range(len(soilHorizon)) for _ in range(horizonDepthT[i], horizonDepthB[i])]
        p_claypct_intpl = [cpt[i] for i in range(len(soilHorizon)) for _ in range(horizonDepthT[i], horizonDepthB[i])]
        p_cfg_intpl = [p_cfg[i] for i in range(len(soilHorizon)) for _ in range(horizonDepthT[i], horizonDepthB[i])]
        ...
        p_bottom_depth = pd.DataFrame({"cokey": [-999], "compname": ["sample_pedon"], "bottom_depth": [0]})
    ...
    data_completeness, text_completeness = compute_data_completeness(bedrock, p_sandpct_intpl, soilHorizon, p_cfg_intpl, rfvDepth, cracks, cr_df)
    ...
    if plot_id is None:
        soilIDRank_output = pd.read_csv("{}/soilIDRank_ofile1.csv".format(current_app.config['DATA_BACKEND']))
        mucompdata = pd.read_csv("{}/soilIDRank_ofile2.csv".format(current_app.config['DATA_BACKEND']))
    ...
    c_bottom_depths = mucompdata_pd[["compname", "c_very_bottom"]].rename(columns={"c_very_bottom": "bottom_depth"})
    ...
    max_depth = 120 if bedrock is None and p_bottom_depth.bottom_depth.values[0] > 120 else p_bottom_depth.bottom_depth.values[0]
    ...
    soil_matrix = pd.DataFrame(np.nan, index=np.arange(max_depth), columns=np.arange(len(slices_of_soil)))
    ...
    if p_hz_data:
        ...
        if p_bottom_depth.bottom_depth.any() > 0:
            horz_vars_group_cokey = [group for _, group in soilIDRank_output_pd.groupby('compname', sort=False)]
            ...
            for group in horz_vars_group_cokey:
                ...
                if subset_group.isnull().values.all():
                    ...
                    Comp_Missing_Status.append('Missing Data' if subset_group.isnull().values.any() else 'Data Complete')
            ...
            for i in soil_matrix.index:
                ...
                if i < bedrock:
                    sample_pedon_slice_vars = sliceT.dropna(axis='columns').drop(columns='compname').columns.to_list()
                    if len(sample_pedon_slice_vars) < 2:
                        sample_pedon_slice_vars = sliceT[sliceT['compname'] == "sample_pedon"].dropna(axis='columns').drop(columns='compname').columns.to_list()
                ...
                for j in range(len(dis_mat)):
                    for k in range(len(dis_mat[j])):
                        if np.isnan(dis_mat[j, k]):
                            if (soil_slice[j] and not soil_slice[k]) or (not soil_slice[j] and soil_slice[k]):
                                dis_mat[j, k] = dis_max
            ...
            D_final_horz = pd.merge(D_final_horz, mucompdata_pd[['compname', 'mukey', 'cokey', 'distance_score', 'Rank_Loc', 'fss']], on='compname', how='left')
    else:
        D_final_horz = pd.concat([
            compnames.reset_index(drop=True),
    ...
    fao74 = ["Acrisols", "Andosols", "Arenosols", "Cambisols", "Chernozems", "Ferralsols", "Fluvisols",
         "Gleysols", "Greyzems", "Histosols", "Kastanozems", "Luvisols", "Nitosols", "Phaeozems", "Planosols",
         "Podzols", "Podzoluvisols", "Regosols", "Solonchaks", "Solonetz", "Vertisols", "Xerosols", "Yermosols"]
    
    fao90 = ["Acrisols", "Alisols", "Andosols", "Anthrosols", "Arenosols", "Calcisols", "Cambisols",
             "Chernozems", "Ferralsols", "Fluvisols", "Gleysols", "Greyzems", "Gypsisols",
             "Histosols", "Kastanozems", "Leptosols", "Lixisols", "Luvisols", "Nitisols",
             "Phaeozems", "Planosols", "Plinthosols", "Podzols", "Podzoluvisols", "Regosols", "Solonchaks",
             "Solonetz", "Vertisols"]
    ...
    if D_final_horz.horz_score.any() > 0 and cr_df.notnull().values.any():
        D_final_horz['Score_Data'] = (D_final_horz.horz_score + (color_sim * color_weight)) / (color_weight + D_final_horz.weight)
        ...
        Rank_Filter = pd.DataFrame({'compname': mucompdata_pd.compname, 'rank_status': rank_status, 'missing_status': missing_status})
    ...
    for comp_grps_temp in D_final_comp_grps:
        comp_grps_temp = comp_grps_temp.sort_values('Score_Data', ascending=False).reset_index(drop=True)
    ...
    # Generate Rank_Data
    D_final_horz = D_final_horz.sort_values(['Score_Data', 'compname'], ascending=[False, True]).reset_index(drop=True)
    rank_id = 1
    Rank_Data = []
    ...
    D_final_loc = pd.merge(D_final_horz, mucompdata_pd[['compname', 'clay', 'distance_score_norm']], on='compname', how='left')
    ...
    Score_Data_Loc = (D_final_loc[['Score_Data', 'distance_score']].sum(axis=1)) / (D_final_loc.weight + 1)
    ...
    for i, row in D_final_loc.iterrows():
        ...
        elif 0 <= bedrock <= 10 and row['fss'].lower() == "fao74" and "lithosols" in row['compname'].lower():
            D_final_loc.at[i, 'Score_Data_Loc'] = 1.001
        elif 0 <= bedrock <= 10 and row['fss'].lower() == "fao90" and "lithic leptosols" in row['compname'].lower():
            D_final_loc.at[i, 'Score_Data_Loc'] = 1.001
        elif 10 < bedrock <= 30 and row['fss'].lower() == "fao74" and ("rendzinas" in row['compname'].lower() or "rankers" in row['compname'].lower()):
            D_final_loc.at[i, 'Score_Data_Loc'] = 1.001
        elif 10 < bedrock <= 30 and row['fss'].lower() == "fao90" and "leptosols" in row['compname'].lower():
            D_final_loc.at[i, 'Score_Data_Loc'] = 1.001
        elif (bedrock is None or bedrock > 50) and any(term in row['compname'].lower() for term in ["lithosols", "leptosols", "rendzinas", "rankers"]):
            D_final_loc.at[i, 'Score_Data_Loc'] = 0.001
    
    D_final_loc['Score_Data_Loc'] = D_final_loc['Score_Data_Loc'] / np.nanmax(D_final_loc['Score_Data_Loc'])
    ...
    for _, comp_grps_temp in D_final_loc.groupby('compname_grp', sort=False):
        comp_grps_temp = comp_grps_temp.sort_values('Score_Data_Loc', ascending=False).reset_index(drop=True)
    ...
    D_final_loc = D_final_loc.sort_values(['Score_Data_Loc', 'compname'], ascending=[False, True]).reset_index(drop=True)
    ...
    D_final_loc = D_final_loc.sort_values(['soilID_rank_final', 'Score_Data_Loc'], ascending=[False, False]).reset_index(drop=True)
    D_final_loc[['Score_Data', 'horz_score', 'Score_Data_Loc', 'distance_score', 'distance_score_norm']] = D_final_loc[['Score_Data', 'horz_score', 'Score_Data_Loc', 'distance_score', 'distance_score_norm']].fillna(0.0)

    # Final scaling of similarity score by 'data_completeness'
    D_final_loc.Score_Data_Loc = D_final_loc.Score_Data_Loc * (0.25 if data_completeness < 25 else float(data_completeness) / 100)
...
def getSoilLocationBasedUS(lon, lat, plot_id):
    ...
    try:
        ...
        out = {"ESD": False,  "OSD_morph": False, "OSD_narrative": False, "hz": False,  "spn": False}
    ...
    if not out['spn']:
        ...
        relevant_columns = ['mukey', 'cokey', 'compname', 'compkind', 'majcompflag', 'comppct_r', 'distance_m', 'slope_r', 'elev_r', 'nirrcapcl', 'nirrcapscl', 'nirrcapunit', 'irrcapcl', 'irrcapscl', 'irrcapunit', 'taxorder', 'taxsubgrp']
    ...
    if data_source == 'SSURGO':
        ...
        muhorzdata_pd[['cokey', 'chkey', 'hzname']] = muhorzdata_pd[['cokey', 'chkey', 'hzname']].astype(str)
        
        # Infill missing CEC values with ECEC
        muhorzdata_pd.cec7_r = np.where(pd.isnull(muhorzdata_pd.cec7_r), muhorzdata_pd.ecec_r, muhorzdata_pd.cec7_r)
        
        # Rename columns for better clarity
        muhorzdata_pd.rename(columns={'cec7_r': 'CEC', 'ph1to1h2o_r': 'pH', 'ec_r': 'EC'}, inplace=True)
        ...
        if muhorzdata_pd[['hzdept_r', 'hzdepb_r']].isnull().all().all() and not mucompdata_pd["compkind"].isin(OSD_compkind).any():
    ...
    for name, count in name_counts.items():
        if count > 1:  # If a component name is duplicated
            ...
            for suffix in suffixes:
                index = component_names.index(name)  # Find the index of the first occurrence of the duplicate name
    ...
    for group in muhorzdata_group_cokey:
        ...
        group_sorted = group_sorted[group_sorted['hzdepb_r'] >= group_sorted['hzdept_r']].reset_index(drop=True)
    ...
    mucompdata_pd = mucompdata_pd.merge(c_bottom_depths[['compname', 'c_very_bottom']], on='compname', how='left')
    ...
    if data_source == 'STATSGO':
        ...
        if mucompdata_pd["compkind"].isin(OSD_compkind).any():
            try:
                ...
                series_name = [urllib.parse.quote(re.sub('[0-9]+', '', compname)) for compname in mucompdata_pd['compname'] if compname in OSD_compkind]
                ...
                columns_to_str = ['series', 'hzname', 'texture_class', 'cf_class', 'matrix_dry_color_hue', 'matrix_dry_color_value', 'matrix_dry_color_chroma']
                OSDhorzdata_pd[columns_to_str] = OSDhorzdata_pd[columns_to_str].astype(str)
                OSDhorzdata_pd[['top', 'bottom']] = OSDhorzdata_pd[['top', 'bottom']].fillna(0).astype(int)
    ...
    nan_columns = pd.DataFrame(np.nan, index=np.arange(len(mucompdata_pd.cokey)), columns=['series', 'brief_narrative'])
    ...
    if OSDhorzdata_pd is not None:
        ...
        OSDhorzdata_pd = OSDhorzdata_pd[OSDhorzdata_pd['cokey'].isin(comp_key)].drop_duplicates().reset_index(drop=True)
        ...
        if mucompdata_pd["compkind"].isin(OSD_compkind).any():
            ...
            def create_new_layer(row, top, bottom):
                ...
                for col in ['hzname', 'texture_class', 'cf_class', 'matrix_dry_color_hue', 'matrix_dry_color_value', 'matrix_dry_color_chroma']:
                    new_row[col] = None
            ...
            for group in OSDhorzdata_group_cokey:
                ...
                if group_sorted['compkind'].isin(OSD_compkind).any() and group_sorted['bottom'].iloc[-1] != 0:
                    ...
                    if group_sorted['top'].iloc[0] != 0:
                        new_layer = create_new_layer(group_sorted.iloc[0], 0, group_sorted['top'].iloc[0])
                        group_sorted = pd.concat([pd.DataFrame([new_layer]), group_sorted], ignore_index=True)
                    
                    # Check for missing subsurface horizons
                    for j in range(len(group_sorted) - 1):
                        if group_sorted['top'].iloc[j + 1] > group_sorted['bottom'].iloc[j]:
                            new_layer = create_new_layer(group_sorted.iloc[j], group_sorted['bottom'].iloc[j], group_sorted['top'].iloc[j + 1])
                            group_sorted = pd.concat([group_sorted.iloc[:j+1], pd.DataFrame([new_layer]), group_sorted.iloc[j+1:]], ignore_index=True)
                    ...
                    if lab_intpl.isnull().values.all():
                        ...
                        munsell_values = [
                            lab2munsell(color_ref, LAB_ref, LAB=lab) if lab[0] and lab[1] and lab[2] else ""
                    ...
                    if OSD_text_int[i] == 'Yes' or OSD_rfv_int[i] == 'Yes':
                        ...
                        OSD_sand_intpl = update_intpl_data(OSD_sand_intpl, ['c_sandpct_intpl', 'c_sandpct_intpl_grp'], sand_values, OSD_very_bottom)
                    
                        clay_values = OSD_clay_intpl.iloc[OSD_very_bottom_int-1].tolist()
                        OSD_clay_intpl = update_intpl_data(OSD_clay_intpl, ['c_claypct_intpl', 'c_claypct_intpl_grp'], clay_values, OSD_very_bottom)
                    
                        rfv_values = OSD_rfv_intpl.iloc[OSD_very_bottom_int-1].tolist()
                        OSD_rfv_intpl = update_intpl_data(OSD_rfv_intpl, ['c_cfpct_intpl', 'c_cfpct_intpl_grp'], rfv_values, OSD_very_bottom)
                        ...
                        # Aggregate sand data
                        snd_d_osd, hz_depb_osd = agg_data_layer(data=OSD_sand_intpl.iloc[:, 0], bottom=OSD_very_bottom, depth=True)
                        ...
                        rf_d_osd = agg_data_layer(data=OSD_rfv_intpl.c_cfpct_intpl, bottom=OSD_very_bottom)
    ...
    if len(cokeys_with_depth) > 0:
        ...
        mucompdata_pd = mucompdata_pd[mucompdata_pd['cokey'].isin(cokeys_with_depth)].reset_index(drop=True)
        muhorzdata_pd = muhorzdata_pd[muhorzdata_pd['cokey'].isin(cokeys_with_depth)].reset_index(drop=True)
        c_bottom_depths = c_bottom_depths[c_bottom_depths['cokey'].isin(cokeys_with_depth)].reset_index(drop=True)
        ...
        layer_lists = [lab_intpl_lyrs, getProfile_cokey, snd_lyrs, cly_lyrs, txt_lyrs, rf_lyrs, 
                       cec_lyrs, ph_lyrs, ec_lyrs, hz_lyrs, lab_lyrs, munsell_lyrs]
        
        for i, lst in enumerate(layer_lists):
            layer_lists[i] = [lst[index] for index in indices_with_depth]
        
        # Unpack the layer lists
        lab_intpl_lyrs, getProfile_cokey, snd_lyrs, cly_lyrs, txt_lyrs, rf_lyrs, \
        cec_lyrs, ph_lyrs, ec_lyrs, hz_lyrs, lab_lyrs, munsell_lyrs = layer_lists
    ...
    if data_source == 'SSURGO':
        if not out['ESD']:
            ...
            ESD[['cokey', 'ecoclassid', 'ecoclassname']] = ESD[['cokey', 'ecoclassid', 'ecoclassname']].astype(str)
            
            # Check if any cokey in ESD matches with mucompdata_pd
            if any(ESD['cokey'].isin(mucompdata_pd['cokey'])):
                ESDcompdata_pd = pd.merge(mucompdata_pd[['mukey', 'cokey', 'compname']], ESD, on='cokey', how='left')
            else:
                ESDcompdata_pd = None
    
    elif data_source == 'STATSGO':
        ESDcompdataQry = 'SELECT cokey,  coecoclasskey, ecoclassname FROM coecoclass WHERE cokey IN (' + ','.join(map(str, cokey_list)) + ')'
        ESDcompdata_out = sda_return(propQry=ESDcompdataQry)
        
        if not ESDcompdata_out:
            ESDcompdata_pd = None
        else:
            ESDcompdata_sda = pd.DataFrame(ESDcompdata_out['Table'][1:], columns=ESDcompdata_out['Table'][0])
            ESDcompdata_sda[['cokey', 'ecoclassid', 'ecoclassname']] = ESDcompdata_sda[['cokey', 'ecoclassid', 'ecoclassname']].astype(str)
            
            # Check if any cokey in ESDcompdata_sda matches with mucompdata_pd
            if any(ESDcompdata_sda['cokey'].isin(mucompdata_pd['cokey'])):
                ESDcompdata_pd = pd.merge(mucompdata_pd[['mukey', 'cokey', 'compname']], ESDcompdata_sda, on='cokey', how='left')
            else:
                ESDcompdata_pd = None
    ...
    soilIDRank_output = [
        pd.concat([getProfile_cokey[i][['compname','sandpct_intpl', 'claypct_intpl', 'rfv_intpl']], lab_intpl_lyrs[i]], axis=1)
    ...
    mucompdata_cond_prob = mucompdata_cond_prob.sort_values(['soilID_rank', 'distance_score_norm'], ascending=[False, False])
    ...
    output_SoilList = [
        dict(zip(["id", "site", "esd", "bottom_depth", "sand", "clay", "texture", 
                  "rock_fragments", "cec", "ph", "ec", "lab", "munsell"], 
                 row)) 
        for row in zip(ID, Site, esd_comp_list, hz_lyrs, snd_lyrs, cly_lyrs, txt_lyrs, 
    ...
    if plot_id is None:
        ...
        saveModelOutput(plot_id, model_version, output_data, 
                        soilIDRank_output_pd.to_csv(index=None, header=True), 
                        mucompdata_cond_prob.to_csv(index=None, header=True))
...
#                                   rankPredictionUS                                                #
#####################################################################################################
def rankPredictionUS(lon, lat, soilHorizon, horizonDepth, rfvDepth, lab_Color, pSlope, pElev, bedrock, cracks, plot_id=None):
...
#------------------------------------------------------------------------------------------------
    ...
    if soil_df_slice is not None:
        ...
        # Create index list of soil slices where user data exists
        pedon_slice_index = [j for i in range(len(soil_df_slice)) for j in range(int(soil_df_slice.top[i]), int(soil_df_slice.bottom[i]))]
        ...
        if not lab_Color.isnull().all():
            lab_Color = [[np.nan, np.nan, np.nan] if x is None else x for x in lab_Color]
            p_lab_intpl = [lab_Color.iloc[i] for i in range(len(lab_Color)) for _ in range(horizonDepthT[i], horizonDepthB[i])]
            p_lab_intpl = pd.DataFrame(p_lab_intpl).reset_index(drop=True)
        else:
            lab_Color = pd.DataFrame([x for x in lab_Color if x is not None])
            p_lab_intpl = pd.DataFrame(np.nan, index=np.arange(120), columns=np.arange(3)).reset_index(drop=True)
        ...
        def adjust_depth_interval(data, target_length=120, add_columns=1):
            ...
            if length > target_length:
                ...
                if add_columns == 1:
                    ...
                    add_data = pd.DataFrame(np.nan, index=np.arange(add_length), columns=np.arange(add_columns))
        ...
        p_hz_data = pd.concat([p_compname, p_sandpct_intpl, p_claypct_intpl, p_cfg_intpl, p_lab_intpl], axis=1)
        p_hz_data.columns = ["compname", "sandpct_intpl", "claypct_intpl", "rfv_intpl", "l", "a", "b"]
    ...
    if pElev is None:
        try:
            ...
            elev_url = f"https://nationalmap.gov/epqs/pqs.php?x={lon}&y={lat}&units=Meters&output=json"
            ...
            pElev = round(elev_data['USGS_Elevation_Point_Query_Service']['Elevation_Query']['Elevation'], 3)
        except Exception as err:
    ...
    missing_data = []
    if text_comp < 40: missing_data.append("soil texture")
    if rf_comp < 25: missing_data.append("soil rock fragments")
    if lab_comp < 15: missing_data.append("soil color")
    if slope_comp < 15: missing_data.append("slope")
    if crack_comp < 5: missing_data.append("soil cracking")
    
    if missing_data:
        missing_text = ", ".join(missing_data)
        text_completeness = f"To improve predictions, complete data entry for: {missing_text} and re-sync."
    else:
    ...
    if plot_id is None:
        # Reading from file
        soilIDRank_output_pd = pd.read_csv(f"{current_app.config['DATA_BACKEND']}/soilIDRank_ofile1.csv")
    ...
    if soilIDRank_output_pd is not None:
        ...
        horz_vars = [p_hz_data]
        horz_vars.extend([group.reset_index(drop=True).loc[pedon_slice_index] for group in horz_vars_group_cokey])
        
        # Calculate similarity for each depth slice
        dis_mat_list = []
        for i in soil_matrix.index: # i should be an index of p_hz_data depth slices, e.g. if user only enters 100-120cm data, then i = 100:120
            ...
            if i < bedrock:
                sample_pedon_slice_vars = sliceT.dropna(axis='columns').drop(['compname'], 1).columns.tolist()
                if len(sample_pedon_slice_vars) < 2:
                    sample_pedon_slice_vars = sliceT[sliceT['compname'] == "sample_pedon"].dropna(axis='columns').drop(['compname'], 1).columns.tolist()
        ...
        dis_mat_nan_check = np.ma.MaskedArray(dis_mat_list, mask=np.isnan(dis_mat_list))
        D_check = np.ma.average(dis_mat_nan_check, axis=0)
        Rank_Filter['rank_status'] = ['Not Ranked' if np.ma.is_masked(x) else 'Ranked' for x in D_check[0][1:]]
        
        # Calculate maximum dissimilarity
        dis_max_slice = [np.nanmax(matrix) if not np.isnan(matrix).all() else np.nan for matrix in dis_mat_list]
        dis_max = np.nanmax(dis_max_slice)
        dis_max_slice = [dis_max if np.isnan(x) else x for x in dis_max_slice]
        
        # Apply depth weight
        depth_weight = np.concatenate((np.repeat(0.2, 20), np.repeat(1., 80)), axis=0)
        ...
        for i, dis_mat in enumerate(dis_mat_list):
            ...
            nan_and_soil_slice_is_one = np.isnan(dis_mat) & np.isin(np.arange(dis_mat.shape[1]), np.where(soil_slice == 1)[0])
            
            # Identify where NaN values exist and both corresponding values in soil_slice are 0
            rows, cols = np.where(np.isnan(dis_mat))
            both_zero_rows_cols = [(row, col) for row, col in zip(rows, cols) if soil_slice[row] == 0 and soil_slice[col] == 0]
        ...
        D_sum = np.ma.average(dis_mat_list, axis=0, weights=depth_weight) if depth_weight is not None else np.ma.average(dis_mat_list, axis=0)
        D_sum = np.ma.filled(D_sum, fill_value=np.nan)
        D_horz = 1 - D_sum
    ...
    def compute_site_similarity(p_slope, mucompdata, slices, additional_columns=None, feature_weight=None):
        """
    ...
    if (pSlope is not None) and (pElev is not None) and (p_bottom_depth.bottom_depth.any() > 0):
        D_site = compute_site_similarity(p_slope, mucompdata_pd, slices_of_soil, 
                                         ['slope_r', 'elev_r', 'bottom_depth'], 
                                         feature_weight=np.array([1., 0.5, 0.5]))
    else:
        D_site = compute_site_similarity(p_slope, mucompdata_pd, slices_of_soil, 
                                         feature_weight=np.array([1., 0.5]))
    ...
    if D_horz is not None and D_site is not None:
        D_site_hz = np.sum([D_site, D_horz], axis=0) / (1 + site_wt)
        D_final = pd.concat([compnames, pd.Series(D_site_hz[0][1:]), pd.Series(np.repeat(1., len(compnames)))], axis=1)
    
    # When only D_horz is available
    elif D_horz is not None:
        D_final = pd.concat([compnames, pd.Series(D_horz[0][1:]), pd.Series(np.repeat(1., len(compnames)))], axis=1)
    
    # When only D_site is available
    elif D_site is not None:
        D_final = pd.concat([compnames, pd.Series(D_site[0][1:]), pd.Series(np.repeat(1., len(compnames)))], axis=1)
    ...
    # When neither D_horz nor D_site is available
        ...
        D_final = pd.concat([compnames, pd.Series(np.repeat(0.0, len(compnames))), pd.Series(np.repeat(0.0, len(compnames)))], axis=1)
    ...
    D_final_loc = pd.merge(D_final, 
                           mucompdata_pd[['compname', 'cokey', 'distance_score', 
                                          'distance_score_norm', 'clay', 'taxorder', 
                                          'taxsubgrp', 'OSD_text_int', 'OSD_rfv_int', 
                                          'data_source', 'Rank_Loc']], 
                           on='compname', 
                           how='left')
    ...
    if D_horz is None and D_site is None:
        ...
        D_final_loc['Score_Data_scale'] = D_final_loc['Score_Data'] / np.nanmax(D_final_loc['Score_Data'])
        D_final_loc['distance_score_scale'] = D_final_loc['distance_score'] / np.nanmax(D_final_loc['distance_score'])
        
        # Calculate the combined score
        Score_Data_Loc = (D_final_loc['Score_Data_scale'] + D_final_loc['distance_score_scale']) / (D_final_loc['data_weight'] + location_weight)
    ...
    D_final_loc[['Score_Data', 'D_horz', 'D_site', 'Score_Data_Loc', 'distance_score', 'distance_score_norm']] = D_final_loc[['Score_Data', 'D_horz', 'D_site', 'Score_Data_Loc', 'distance_score', 'distance_score_norm']].fillna(0.0)
    ...
    Rank = [
        {
            ...
            "component": row.compname_grp.capitalize(),
            "componentID": row.cokey,
            "score_data_loc": "" if row.missing_status == "Location data only" else round(row.Score_Data_Loc, 3),
            "rank_data_loc": "" if row.missing_status == "Location data only" else row.Rank_Data_Loc,
            "score_data": "" if row.missing_status == "Location data only" else round(row.Score_Data, 3),
...
def getSoilGridsGlobal(lon, lat, plot_id=None):
    ...
    if sg_data_w[['sand', 'clay', 'cfvo']].isnull().all().all():
        ...
        component_keys = [
            'compname', 'probability', 'descriptionEN', 'managementEN', 'descriptionES', 
            'managementES', 'descriptionKS', 'managementKS', 'descriptionFR', 'managementFR'
        ]
        component_values = [
            TAXNWRB_pd.WRB_tax, TAXNWRB_pd.Prob, TAXNWRB_pd.Description_en, TAXNWRB_pd.Management_en,
            TAXNWRB_pd.Description_es, TAXNWRB_pd.Management_es, TAXNWRB_pd.Description_ks, 
            TAXNWRB_pd.Management_ks, TAXNWRB_pd.Description_fr, TAXNWRB_pd.Management_fr
        ]
        
        # Create 'components' dictionary using a loop
        components_dict = {k: dict(zip(TAXNWRB_pd.index, v)) for k, v in zip(component_keys, component_values)}
        ...
        remaining_values = [lpks_depths, texture_pd_lpks, sand_pd_lpks, clay_pd_lpks, rfv_pd_lpks, pH_pd_lpks, cec_pd_lpks]
        ...
        SoilGrids.update({k: dict(list(zip(v.index, v))) for k, v in zip(remaining_keys, remaining_values)})
        ...
        if plot_id is not None:
            saveSoilGridsOutput(plot_id, model_version, json.dumps({"metadata": metadata, "soilGrids": SoilGrids}))
