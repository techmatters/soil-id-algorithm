...
def aggregate_data_vi(data, max_depth, sd=2):
    ...
    for column in data.columns:
        ...
        results.append(
            pd.DataFrame(
                column_results, columns=["hzdept_r", "hzdepb_r", f"Aggregated Data ({column})"]
            )
        )
    ...
    for _, row in rep_columns.iterrows():
        ...
        simulated_txt = acomp(
            simulate_correlated_triangular(
                row["score_loc"] * 1000, params_txt, texture_correlation_matrix
            )
        )
        ...
        sim_data = simulate_correlated_triangular(
            row["score_loc"] * 1000, params, local_correlation_matrix
        )
