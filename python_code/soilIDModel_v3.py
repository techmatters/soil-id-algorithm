...
def getSoilLocationBasedUS(lon, lat, plot_id):
    ...
    if not out["spn"]:
        ...
        mu_id_dist = pd.DataFrame(
            {"MUKEY": mu_geo["MUKEY"].values, "distance": distances, "pt_intersect": intersects}
        )
    ...
    for group in muhorzdata_group_cokey:
        ...
        for l in range(len(snd_d)):
            text_T = getTexture(
                row=None, sand=snd_d[l], silt=(100 - (snd_d[l] + cly_d[l])), clay=cly_d[l]
            )
            txt_d.append(text_T)
        txt_d = pd.Series(txt_d)
        txt_d.index = snd_d.index
        rf_d = agg_data_layer(
            data=cf_pct_intpl["c_cfpct_intpl"], bottom=c_bottom_depths_temp["c_very_bottom"].iloc[0]
        )
        cec_d = agg_data_layer(
            data=cec_intpl["c_cec_intpl"], bottom=c_bottom_depths_temp["c_very_bottom"].iloc[0]
        )
        ph_d = agg_data_layer(
            data=ph_intpl["c_ph_intpl"], bottom=c_bottom_depths_temp["c_very_bottom"].iloc[0]
        )
        ec_d = agg_data_layer(
            data=ec_intpl["c_ec_intpl"], bottom=c_bottom_depths_temp["c_very_bottom"].iloc[0]
        )
    ...
    if data_source == "STATSGO":
        ...
        if mucompdata_pd["compkind"].isin(OSD_compkind).any():
            try:
                ...
                OSDhorzdata_pd.loc[
                    OSDhorzdata_pd["matrix_dry_color_hue"] == "N", "matrix_dry_color_value"
                ] = 1
    ...
    nan_columns = pd.DataFrame(
        np.nan, index=np.arange(len(mucompdata_pd.cokey)), columns=["series", "brief_narrative"]
    )
                ...
                if (
                    ...
                    if lab_intpl.isnull().values.all():
                        ...
                        l_d, osd_hz_d = agg_data_layer(
                            data=lab_intpl["l"], bottom=OSD_very_bottom, sd=4, depth=True
                        )
                        a_d = agg_data_layer(
                    ...
                    if OSD_text_int[i] == "Yes" or OSD_rfv_int[i] == "Yes":
                        ...
                        snd_d_osd, hz_depb_osd = agg_data_layer(
                            data=OSD_sand_intpl.iloc[:, 0], bottom=OSD_very_bottom, depth=True
                        )
                    ...
                    lab_intpl = pd.DataFrame(
                        np.nan, index=np.arange(c_bottom_depths.iloc[i, 2]), columns=["l", "a", "b"]
                    )
                    lab_intpl_lyrs.append(lab_intpl)
        ...
        if not out["ESD"]:
            ...
            if any(ESD["cokey"].isin(mucompdata_pd["cokey"])):
                ESDcompdata_pd = pd.merge(
                    mucompdata_pd[["mukey", "cokey", "compname"]], ESD, on="cokey", how="left"
                )
            else:
                ESDcompdata_pd = None
